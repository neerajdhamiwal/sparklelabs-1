(function($, Drupal, document) {
      ...
      new OffAmazonPayments.Widgets.AddressBook({
        ...
        getContractId: function () {
          if (Drupal.AmazonLPA.orderReferenceId) {
            console.log("Reused order reference ID: " + Drupal.AmazonLPA.orderReferenceId);
        ...
        onOrderReferenceCreate: function (orderReference) {
          if (!Drupal.AmazonLPA.orderReferenceId) {
            ...
            console.log("Generated order reference ID: " + Drupal.AmazonLPA.orderReferenceId);
            var $referenceIdField = $('input[name="commerce_amazon_lpa_contract_id[reference_id]"]', document);
            if ($referenceIdField.length > 0) {
              console.log("Generated saved order reference ID: " + Drupal.AmazonLPA.orderReferenceId);
    ...
    WalletWidget: function (elId) {
      ...
      new OffAmazonPayments.Widgets.Wallet({
        ...
        onOrderReferenceCreate: function(orderReference) {
          ...
          if ($referenceIdField.length > 0 && $referenceIdField.val() === 0) {
            $referenceIdField.val(orderReference.getAmazonOrderReferenceId());
          }
        },
        design: {
  ...
  Drupal.behaviors.commerceAmazonLPA = {
    attach: function (context, settings) {
      $.extend(Drupal.AmazonLPA, settings.AmazonLPA);
      ...
      // For any following attachments, run initialization.
      else {
        Drupal.AmazonLPA.initialize(settings);
      }
    }
